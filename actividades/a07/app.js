...
function bindUI() {
    ...
    $("#product-form").on("submit", e => {
        ...
        $.ajax({
            url: id ? "./backend/update.php" : "./backend/insert.php",
            method: "POST",
    ...
    $("#products").on("click", ".btn-del", function() {
        ...
        $.get("./backend/delete.php", {id}, resp => {
            const r = typeof resp === "string" ? JSON.parse(resp) : resp;
            barra(`status: ${r.status}`, `message: ${r.message}`);
            listar();
...
function buscar(q) {
    ...
    $.get("./backend/singleByName.php", {search: q}, data => {
        const productos = typeof data === "string" ? JSON.parse(data) : data;
        renderTabla(productos);
